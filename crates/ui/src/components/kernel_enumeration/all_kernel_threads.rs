//! All Kernel Threads Enumeration sub-tab

use callback::{
    enumerate_all_kernel_threads, resolve_ethread_offsets, resolve_kernel_symbol, resume_thread,
    set_ethread_offsets, suspend_thread, terminate_thread, KernelThreadInfo,
};
use dioxus::prelude::*;

use super::SortOrder;

/// Sort column for kernel threads
#[derive(Clone, Copy, PartialEq, Debug)]
enum KernelThreadSortColumn {
    ProcessId,
    ThreadId,
    StartAddress,
    State,
...
/// All Kernel Threads Enumeration sub-tab
#[component]
pub fn AllKernelThreadsTab(driver_loaded: bool) -> Element {
    let mut threads = use_signal(|| Vec::<KernelThreadInfo>::new());
    let mut status_message = use_signal(|| String::new());
    let mut is_enumerating = use_signal(|| false);
    let mut sort_column = use_signal(|| KernelThreadSortColumn::ProcessId);
    ...
    let handle_enumerate = move |_| {
        ...
        spawn(async move {
            ...
            status_message.set("Enumerating all kernel threads...".to_string());

            // Enumerate all kernel threads
            let result = tokio::task::spawn_blocking(enumerate_all_kernel_threads).await;

            match result {
                Ok(Ok(thread_list)) => {
                    let count = thread_list.len();
                    threads.set(thread_list);
                    status_message.set(format!("Found {} kernel threads", count));
                }
                Ok(Err(e)) => {
    ...
    // Sort threads
    let sorted_threads: Vec<KernelThreadInfo> = {
        ...
        if !query.is_empty() {
            list.retain(|t| {
                t.module_name_str().to_lowercase().contains(&query)
                    || format!("{}", t.thread_id).contains(&query)
                    || format!("{}", t.process_id).contains(&query)
            });
        }
        ...
        list.sort_by(|a, b| {
            let cmp = match col {
                KernelThreadSortColumn::ProcessId => a.process_id.cmp(&b.process_id),
                KernelThreadSortColumn::ThreadId => a.thread_id.cmp(&b.thread_id),
                KernelThreadSortColumn::StartAddress => a.win32_start_address.cmp(&b.win32_start_address),
                KernelThreadSortColumn::State => a.state.cmp(&b.state),
            };
    ...
    // Sort indicator
    let sort_indicator = |col: KernelThreadSortColumn| -> &'static str {
    ...
    // Make sort handler
    let make_sort_handler = move |col: KernelThreadSortColumn| {
        move |_| {
            if *sort_column.read() == col {
        ...
        div { class: "callback-enum-content",
            ...
            div { class: "controls",
                input {
                    class: "search-input",
                    r#type: "text",
                    placeholder: "Search by module name, PID, or TID...",
                    value: "{query_text}",
                ...
                table { class: "process-table",
                    thead { class: "table-header",
                        tr {
                            th {
                                class: "th sortable",
                                onclick: make_sort_handler(KernelThreadSortColumn::ProcessId),
                                "PID{sort_indicator(KernelThreadSortColumn::ProcessId)}"
                            }
                            th {
                                class: "th sortable",
                                onclick: make_sort_handler(KernelThreadSortColumn::ThreadId),
                                "TID{sort_indicator(KernelThreadSortColumn::ThreadId)}"
                            }
                            th {
                                class: "th sortable",
                                onclick: make_sort_handler(KernelThreadSortColumn::StartAddress),
                                "Start Address{sort_indicator(KernelThreadSortColumn::StartAddress)}"
                            }
                            th {
                                class: "th sortable",
                                onclick: make_sort_handler(KernelThreadSortColumn::State),
                                "State{sort_indicator(KernelThreadSortColumn::State)}"
                            }
                            th { class: "th", "Wait" }
                            th { class: "th", "Actions" }
                        }
                    ...
                    tbody {
                        if sorted_threads.is_empty() && !threads.read().is_empty() {
                            ...
                            tr {
                                td { colspan: "6", class: "no-results",
                                    if driver_loaded {
                                        "Click 'Refresh' to enumerate all kernel threads"
                                    } else {
                            ...
                            for thread in sorted_threads.iter() {
                                {
                                    ...
                                    let module = thread.module_name_str().to_string();
                                    // Resolve symbol for ntoskrnl addresses
                                    let start_address_str = if !module.is_empty() {
                                        resolve_kernel_symbol(start_addr, &module, thread.module_base)
                                    } else {
                                    ...
                                    rsx! {
                                        tr {
                                            ...
                                            td { class: "cell",
                                                span { class: if is_system { "cpu-medium" } else { "" }, "{pid}" }
                                            }
                                            td { class: "cell", "{tid}" }
            ...
            if total_count > 0 {
                div { class: "table-footer",
                    "Showing {thread_count} of {total_count} kernel threads"
                }
            }
        }
    }
