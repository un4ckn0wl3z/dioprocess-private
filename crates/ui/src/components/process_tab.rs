...
use super::{
    ...
    TokenThiefWindow, apply_dph_file_to_process, apply_dpr_file_to_process,
};
                    ...
                    div {
                        ...
                        div {
                                ...
                                onclick: move |_| {
                                    ...
                                    if let Some(pid) = target_pid {
                                        match patch_etw(pid) {
                                            Ok(()) => {
                                                status_message.set(format!(
                                                    "✓ ETW patched in process {} — EtwEventWrite bypassed",
                                                    pid
                                                ));
                                            }
                                            Err(e) => {
                            ...
                            button {
                                    ...
                                    if let Some(pid) = target_pid {
                                        spawn(async move {
                                            let file = rfd::AsyncFileDialog::new()
                                                ...
                                                .set_title("Select .dpr Script to Apply")
                                                .pick_file()
                                                .await;

                                            if let Some(file) = file {
                                                let path = file.path().to_string_lossy().to_string();
                                                match apply_dpr_file_to_process(pid, &path) {
                                                    Ok(msg) => {
                                                        status_message.set(format!("✓ {}", msg));
                                                    }
                                                    Err(e) => {
                                                        status_message.set(format!("✗ .dpr script failed: {}", e));
                                                    }
                                                }
                                                spawn(async move {
                                                    tokio::time::sleep(std::time::Duration::from_secs(5)).await;
                                ...
                                span { "📋" }
                                span { "Apply .dpr Script" }
                            }

                            div { class: "context-menu-separator" }

                            // Kernel (Ring 0) submenu - requires driver
                            div {
                                class: "context-menu-submenu",
                                div {
                            ...
                            // Hypervisor (Ring -1) submenu - requires hypervisor
                            div {
                                class: "context-menu-submenu",
                                div {
                                    class: if hv_is_running() { "context-menu-submenu-trigger" } else { "context-menu-submenu-trigger disabled" },
                                    span { "💀" }
                                    span { "Hypervisor (Ring -1)" }
                                    span { class: "arrow", "▶" }
                                }
                                div {
                                    class: "context-menu-submenu-content",
